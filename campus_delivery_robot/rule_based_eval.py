import numpy as np
import rclpy
from campus_delivery_robot.campus_env import (
    CampusEnv, DELIVERY_POINTS, CHARGING_STATION, PICKUP_POINT
)
...
def evaluate(n_episodes=10):
    ...
    for ep in range(n_episodes):
        ...
        print(f"[Rule] 에피소드 {ep+1:2d}: 배달={ep_delivered:2d}, 배터리={env.battery:.1f}%")

    print(f"\n=== Rule-Based 평가 결과 ({n_episodes}에피소드) ===")
