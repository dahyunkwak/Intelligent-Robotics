...
CHARGING_STATION = np.array([16.0, -20.0])   # v3: 맵 동남쪽 외딴 곳
...
#  환경 클래스 (v3)
# ─────────────────────────────────────────────
class CampusEnvV3(gym.Env):
    """
    캠퍼스 배달 로봇 환경 v3
    - 배달지 4개 (v1과 동일)
    - 충전소 위치: 맵 북서쪽 외딴 곳 (-20, 12)
    - 충전 비용이 커서 RL이 더 유리한 환경
    ...
    def _navigate_to(self, x: float, y: float) -> bool:
        ...
        for attempt in range(NAV_RETRY_COUNT):
            ...
            print(f"[NAV] ⚠️ 목표 거부됨 ({attempt+1}/{NAV_RETRY_COUNT}), 재시도...")
        ...
        self._parse_states()
        self._last_nav_dist += float(np.linalg.norm(self.robot_pos - _track_pos))
        dist_to_goal = float(np.linalg.norm(self.robot_pos - np.array([x, y])))
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        return np.array([
            ...
            float(self.delivered[1]), float(self.delivered[2]),
            float(self.delivered[3]), float(self.delivered[4]),
            float(self.carrying[1]),  float(self.carrying[2]),
            float(self.carrying[3]),  float(self.carrying[4]),
            *dists, dist_charger, dist_pickup,
        ], dtype=np.float32)
    ...
    def step(self, action: int):
        ...
        return obs, reward, terminated, truncated, {'total_delivered': self.total_delivered}
...
if __name__ == '__main__':
    env = DummyVecEnv([lambda: ActionMasker(CampusEnvV3(), lambda e: e.action_masks())])
