import numpy as np
import rclpy
from campus_delivery_robot.campus_env_v2 import (
    CampusEnvV2, DELIVERY_POINTS, CHARGING_STATION, PICKUP_POINT
)
...
def rule_based_action(env):
    ...
    """
    if env.battery < 30:
        return 7  # 충전소 (v2에서는 action 7)

    carrying = [i for i in range(1, 7) if env.carrying[i]]
    if carrying:
        dists = {
            i: np.linalg.norm(env.robot_pos - DELIVERY_POINTS[i])
            for i in carrying
        }
        return min(dists, key=dists.get) - 1  # action index (0~5)

    return 6  # 정문 (v2에서는 action 6)
...
def evaluate(n_episodes=10):
    ...
    for ep in range(n_episodes):
        ...
        for step in range(30):
            ...
            # 불필요한 충전 카운트 (배터리 80% 이상인데 충전소 가는 경우)
            if action == 7 and env.battery > 80:
        ...
        print(f"[Rule-v2] 에피소드 {ep+1:2d}: 배달={ep_delivered:2d}, 배터리={env.battery:.1f}%")

    print(f"\n=== Rule-Based v2 평가 결과 ({n_episodes}에피소드) ===")
