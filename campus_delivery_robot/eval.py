...
def evaluate(n_episodes=10):
    ...
    env = VecNormalize.load(
        "/home/dahyun/ros2_ws/src/campus_delivery_robot/ppo_campus_vecnormalize.pkl",
        env
    )
    env.training = False
    ...
    model = MaskablePPO.load(
        "/home/dahyun/ros2_ws/src/campus_delivery_robot/ppo_campus_robot",
        env=env
    )
    ...
    for ep in range(n_episodes):
        ...
        raw_env = env.envs[0].env
        print(f"[RL] 에피소드 {ep+1:2d}: 배달={ep_delivered:2d}, 배터리={raw_env.battery:.1f}%")

    print(f"\n=== RL 모델 평가 결과 ({n_episodes}에피소드) ===")
